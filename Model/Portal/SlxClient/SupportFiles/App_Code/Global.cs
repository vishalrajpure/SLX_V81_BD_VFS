...
<body>
  <div class=""msg"">
    ...
    /// </summary>
    /// <returns>
    ///   <c>ErrorHelper.ResultKind.True</c> if the current user is authenticated; otherwise, <c>ErrorHelper.ResultKind.False</c>.
    /// If the ApplicationContext.Current is null <c>ErrorHelper.ResultKind.Ambiguous</c> is returned.
    ///</returns>
    private static ErrorHelper.ResultKind IsAuthenticated()
    {
        ...
        try
        {
            if (ApplicationContext.Current != null && ApplicationContext.Current.Services != null)
            {
                ...
                result = authProvider != null ? (authProvider.IsAuthenticated ? ErrorHelper.ResultKind.True : ErrorHelper.ResultKind.False) : ErrorHelper.ResultKind.False;
            }
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Gets the job notification poll interval.
    /// </summary>
    public static int JobNotificationPollInterval
    {
        get
        {
            int jobPollInterval;
            return int.TryParse(WebConfigurationManager.AppSettings["JobNotificationPollInterval"], out jobPollInterval)
                       ? jobPollInterval
                       : 5;
        }
    }

        ...
        try
        {
            ...
            if (mergeProvider != null)
            {
                ...
                if (mergeProvider.Target.EntityType == typeof (IContact))
                {
                    ...
                    //this is currently being accessed in the BR extensions in order to update relationships on a lead which has been integration
                    var sessionService = ApplicationContext.Current.Services.Get<ISessionService>(true);
                    var sessionState = sessionService.GetSessionState();
                    sessionState.Add("LeadMergeProvider", mergeProvider);

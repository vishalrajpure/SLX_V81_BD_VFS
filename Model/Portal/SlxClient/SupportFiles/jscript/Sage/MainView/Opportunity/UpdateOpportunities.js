...
function (declare, i18nStrings, _DialogHelpIconMixin, dojoLang, dijitDialog, _Widget, _Templated, Dialogs, Lookup, template, jobs, dojoString) {
    var updateOpportunities = declare('Sage.MainView.Opportunity.UpdateOpportunities', [_Widget, _Templated], {
        ...
        _okClick: function () {
            ...
            var parameters = [
                ...
                { "name": "BusinessRuleName", "value": this.getBusinessRuleMethod() }
            ];

            var options = {
                closable: true,
                title: i18nStrings.updateMultipleOpps_Caption,
            ...
            if (this.propertyNameSelect) {
                ...
                case 'EstClose':
                    if (this.rdoEstClose.get('checked')) {
                        ...
                        return this.moveEstCloseDateList.value == "Forward" ? { name: 'EstimatedClose', value: this.daysToMove.value } :
                            { name: 'EstimatedClose', value: dojoString.substitute('-${0}', [this.daysToMove.value]) };
                    }
                    break;
                case 'Comments':
        ...
        getBusinessRuleMethod: function() {
            return this.propertyNameSelect.value === 'EstClose' && !this.rdoEstClose.get('checked') ? 'UpdateEstimatedCloseDate' : '';
        },
        _close: function() {
            this._dialog.hide();
        }

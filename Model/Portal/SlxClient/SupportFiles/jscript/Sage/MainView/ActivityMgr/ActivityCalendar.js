    ...
    var activityCalendar = declare('Sage.MainView.ActivityMgr.ActivityCalendar', null, {
        ...
        _updateDateRange: function (data, initial) {
            ...
            if (this._dateRangeStart && this._dateRangeEnd) {
                ...
                switch (data.mode) {
                    case 'day':
                        if (dojo.date.compare(nDate, eDate, "date") > 0 || (dojo.date.compare(sDate, nDate, "date") > 0)) {
                            resetQuery = true;
                        }
                        break;
                    case 'week':
        ...
        _formatDateToCompare: function (dt) {
            var d = dt;
            if (Sage.Utility.Convert.isDateString(d)) {
                d = Sage.Utility.Convert.toDateFromString(d);
            }
        ...
        _refreshSchedulerEvents: function () {
            ...
            if (this._selectedUsers) {
                ...
                if (this._selectedUsers[cUserId]) {
                    showHistory = this._selectedUsers[cUserId]["loadHistoryOnStart"];
                }
        ...
        _getActivities: function (userId, color, start, count) {
            if (userId == null) {
            ...
            if (!this.activitiesStore) {
                this.activitiesStore = writableSDataStore({
                    ...
                    sort: [{ attribute: 'StartDate'}]//, descending: false}]
                });
            }
            ...
            this.activitiesStore.fetch({
                ...
                onBegin: this._onActivitiesRequestBegin,
                scope: scopeObj,
                count: count,
                start: start
            });
        },
        _onActivitiesRequestBegin:function(totalResults,contextscope){                
            ...
            if (totalResults > 100 && totalResults > (start + count)) {
                if (totalResults <= 300 && start == 1) {                    
                    _this._getActivities(_queryUserId, color, (start + count) - 1, totalResults);
                } else {                   
                    start = start == 1 ? (start + count) - 1 : start + count;
                    _this._getActivities(_queryUserId, color, start, 100);
                }
            }
        ...
        _getHistory: function (userId, color, start, count) {
            ...
            this.historyActivitiesStore.fetch({
                ...
                onBegin: this._onHistoryRequestBegin,
                scope: scopeObj,
                count: count,
                start: start
            });
        },
        _onHistoryRequestBegin: function (totalResults, contextscope) {            
            var _this = contextscope.scope.me;
            var _queryUserId = contextscope.scope.userId;
            var count = contextscope.scope.count;
            var start = contextscope.scope.start;
            ...
            if (totalResults > 100 && totalResults > (start + count)) {
                if (totalResults <= 300 && start == 1) {                   
                    _this._getHistory(_queryUserId, color, (start + count) - 1, totalResults);
                } else { 
                    start = start == 1 ? (start + count) - 1 : start + count;
                    _this._getHistory(_queryUserId, color, start, 100);
                }
            }
        ...
        _receivedHistoryItem: function (historyData) {
            ...
            if (historyData != null && historyData != undefined) {
                ...
                if (this._selectedUsers[userId]) {
                    var showHistory = false;
                    if (this._selectedUsers[userId]) {
                        showHistory = this._selectedUsers[userId]["loadHistoryOnStart"];
                    }
                    if (showHistory) {
                        var histObj = {};
        ...
        _formatActivityObject: function (actJson, userId, status, loggedInUser) {
            ...
            if (actJson != null) {
                ...
                if (Sage.Utility.Convert.isDateString(actJson.StartDate)) {
                    actJson.StartDate = Sage.Utility.Convert.toDateFromString(actJson.StartDate);
                }
                var stDate = new Date(actJson.StartDate.getFullYear(), actJson.StartDate.getMonth(), actJson.StartDate.getDate(), actJson.StartDate.getHours(), actJson.StartDate.getMinutes());
                var eDate = dojo.date.add(stDate, "minute", actObj.duration);
                if (dojo.date.compare(eDate, stDate, "date") > 0) {
                    actObj.endsAfterMidnight = true;
                    actObj.actualEndDate = eDate; 
                }
        ...
        _formatHistoryObject: function (userId, historyData, loggedInUser) {
            ...
            if (!historyData.Timeless) {
                ...
                historyActivityObj.start_date = utility.Activity.formatActivityEndDateForCalendar(historyData.CompletedDate, historyActivityObj.duration * -1);
                historyActivityObj.end_date = utility.Activity.formatActivityStartDateForCalendar(historyData.CompletedDate);
            }
            return historyActivityObj;

        },
        _onHistoryRequestComplete: function (historyData, contextscope) {
            if (historyData != null && historyData != undefined) {
                ...
                for (var i = 0; i < historyData.length; i++) {
                    ...
                    historyActivityObj = _this._formatHistoryObject(currentUserId, historyData[i], loggedInUser);
                    if (!_this._isEmptyObject(historyActivityObj)) {
                ...
                if (currentUserId != null) {
                    ...
                    _this._userActivityData[currentUserId]["history"] = schedulerEventsHistory;

                    dojo.publish('/entity/activity/loadScheduler', [dojo.clone(schedulerEventsHistory), _this]);
                }
            }

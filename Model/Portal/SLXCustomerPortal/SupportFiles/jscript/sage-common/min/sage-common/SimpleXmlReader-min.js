...
Sage.SimpleXmlReader=function(xml){this.xmlDocument=null;if(typeof xml!="undefined"){this.loadXml(xml)}};Sage.SimpleXmlReader.prototype.getNodeText=function(node){return(dojo.isIE)?node.text:node.textContent;};Sage.SimpleXmlReader.prototype.loadXml=function(xml){this.xmlDocument=null;if(dojo.isIE){var oXmlDocument=new ActiveXObject("Microsoft.XMLDOM");oXmlDocument.async=false;oXmlDocument.loadXML(xml);this.xmlDocument=oXmlDocument;return true;}
...
return false;};Sage.SimpleXmlReader.prototype.selectChildNodes=function(path){if(dojo.isIE){return this.xmlDocument.selectNodes(path);}
...
return arrNodes;}
return null;};Sage.SimpleXmlReader.prototype.selectSingleNode=function(path,node){if(dojo.isIE){if(node){return node.selectSingleNode(path);}
...
return null;};Sage.SimpleXmlReader.prototype.selectSingleNodeText=function(path,node){var oNode=this.selectSingleNode(path,node);if(oNode!=null){return(dojo.isIE)?oNode.text:oNode.textContent;}
